...
from scrapy_splash.dupefilter import splash_fingerprint
...
def test_dict_hash_non_strings():
    h1 = dict_hash(
        {"foo": "bar", "float": 1.1, "int": 2, "bool": False, "seq": ["x", "y", (2, 3.7, {"x": 5, "y": [6, 7]})]}
    )
...
def test_request_fingerprint_nosplash():
    ...
    assert request_fingerprint(r1) == splash_fingerprint(r1)
    assert request_fingerprint(r1) == request_fingerprint(r2)
    assert request_fingerprint(r1) == splash_fingerprint(r2)


def assert_fingerprints_match(r1, r2):
    assert splash_fingerprint(r1) == splash_fingerprint(r2)
...
def test_duplicates(i, dupe_indices, requests, splash_mw_process):
    ...
    def assert_filtered(r1, r2):
        ...
        fp1 = splash_fingerprint(r1)
        fp2 = splash_fingerprint(r2)
        if fp1 != fp2:
